...
export function HomeView({
  ...
  return (
    <div className="main-content">
      <PageHeader title="League Standings" badge={seasonLabel} badgeClassName="season-badge-red" />
      <SummaryCards
...
export function PlayersView({ leaderboard, onBack, onSelectPlayer }) {
  return (
    <div className="main-content">
      ...
      <StandingsCard>
        <LeaderboardTable rows={leaderboard} onSelectPlayer={onSelectPlayer} splitTopThree />
      </StandingsCard>
    </div>
  )
}

export function SessionsView({ games, onBack, onOpenGame, onSelectPlayer, playerRanks = new Map() }) {
  return (
    <div className="main-content">
      <LinkButton onClick={onBack}>Back to Dashboard</LinkButton>
...
export function GameView({ game, onBack, onSelectPlayer, playerRanks = new Map(), backLabel = 'Back to Sessions' }) {
  return (
    <div className="main-content">
              ...
              <tr key={player.id}>
                <td data-label="Player">
                  <PlayerNameCell
                    name={player.player}
                    onSelect={onSelectPlayer}
                    rank={playerRanks.get(player.player)?.rank}
                    rankTotal={playerRanks.get(player.player)?.total}
                  />
                </td>
...
export function PlayerView({ profile, onBack, onOpenGame }) {
  ...
  const champion = isS1ChampionPlayer(player)
  const headingClassName = [
    champion ? 'player-heading-champion' : playerRankTone(summary.powerRank),
  ]
    .filter(Boolean)
    .join(' ')
  let streak = 0
      ...
      <div className="player-hero">
        ...
        <div className="player-info">
          <div className="player-title-row">
            <h2 className={headingClassName}>{player}</h2>
            {champion ? <span className="champion-tag champion-tag-large">S1 Champion</span> : null}
          </div>
          <div className="badge">
