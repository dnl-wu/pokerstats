import { DISPLAY_SMALL_BLIND, PLAYER_NAME_ALIASES } from './constants.js'
...
function normalizePlayerName(name) {
  ...
  return PLAYER_NAME_ALIASES[trimmedName] ?? trimmedName
}

export function normalizeSessions(rows) {
  return rows
    .filter((row) => getRowValue(row, 'Player', 'Name') && row['Game name'])
    .map((row, index) => {
      const player = normalizePlayerName(getRowValue(row, 'Player', 'Name'))
      const gameName = row['Game name']

      return {
        id: `${gameName}-${row.Date}-${player}-${index}`,
        player,
        gameName,
...
export function buildLeaderboard(rows) {
  ...
  rows.forEach((row, index) => {
    const statsPlayer = normalizePlayerName(getRowValue(row, 'Player', 'Player_2'))
    const powerPlayer = normalizePlayerName(getRowValue(row, 'Player_1'))
